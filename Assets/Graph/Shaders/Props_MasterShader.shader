...
////// Emissive:
                ...
                float node_6933 = floor(max(0,dot(lightDirection,normalDirection)) * _Bands) / (_Bands - 1);
                float3 finalColor = emissive + (((node_544*node_6933)+((node_6933*floor(max(0,dot(normalDirection,halfDirection)) * _Bands) / (_Bands - 1))*0.5))*_LightColor0.rgb*attenuation);
...
////// Lighting:
                ...
                float3 node_544 = (_Diffuse_var.rgb*_Color.rgb); // Diffuse Color
                float node_6933 = floor(max(0,dot(lightDirection,normalDirection)) * _Bands) / (_Bands - 1);
                float3 finalColor = (((node_544*node_6933)+((node_6933*floor(max(0,dot(normalDirection,halfDirection)) * _Bands) / (_Bands - 1))*0.5))*_LightColor0.rgb*attenuation);
